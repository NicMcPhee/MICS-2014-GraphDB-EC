...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Talk about Symbolic Regression
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
In our system, individuals contain two items: a function called the GpTree, and the GpTree's fitness. GpTrees are represented in prefix notation by arrays containing variables, constants, and operators. Prefix notation is a way to write  a function that places the operator before its arguments. For example, a GpTree of the function $x + (x * 4)$ would be represented by the following array: [+,~x,~*,~x,~4]. The GpTree's fitness is the sum of the absolute error between the target function and the GpTree at all predetermined variable values. The absolute error is  the measured value of a quantity $x_{0}$ and its actual value $x$, given by the equation $\Delta x=x_{0}-x$.
