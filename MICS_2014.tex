...
To create the initial population, we implemented an algorithm call PTC2~\cite{Luke2013Metaheuristics}. The PTC2 algorithm creates trees by randomly adding operators to an array (leaving blank indices where appropriate for arguments) until a specified length is reached. The blank indices are then filled by leaves (variables and constants). In all runs of our algorithm, leaves consisted of 63\% variables and 37\% constants.
